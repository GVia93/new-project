...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Некорректные данные
        ("Visa Platinum ABCD567890123456", None),  # Буквы в номере карты
        ("Счет 1234ABCD5678EFGH9012", None),  # Буквы в номере счета
        ("Visa 12345", None),  # Слишком короткий номер
        ("Mastercard 123456789012345678901", None),  # Слишком длинный номер
        ("", None),  # Пустая строка
        ("Счет", None),  # Только "Счет" без номера
        ("Visa Platinum", None),  # Только название карты
        ("1234567890123456", None),  # Отсутствует название карты или "Счет"
    ],
)
